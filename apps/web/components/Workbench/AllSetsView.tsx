...
export const AllSetsView: React.FC<AllSetsViewProps> = () => {
  ...
  const cardSets = useMemo(() => data?.map(transformCardSetOutput), [data]);
  ...
  const onSave = useCallback(
    async (cardSet: MseCardSet) => {
      let newSet = await addSet(transformCardSetInput(cardSet, 'insert'));
      if (newSet) {
        setIsNewDialogOpen(false);
    ...
    [addSet, transformCardSetInput, router]
  );
