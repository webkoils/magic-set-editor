...
import { mtgSymbolClasses, mtgSymbolShadow } from '../mtgSymbolClasses';
...
> = ({ children, shadow, ...props }) => {
  ...
  const { childSymbols, className } = useMemo(() => {
    const splitChildren = String(children).split('/').slice(0, 3);
    const dimensionsForSymbolSplit = dimensions[splitChildren.length - 1];
    ...
    const symbols = splitChildren.map((sym, i) => {
  ...
  return (
    <svg
      ...
      style={{
        ...
        marginRight: '.05em',
        marginBottom: '-.125em',
        filter: shadow ? mtgSymbolShadow : undefined,
      }}
      {...props}
      viewBox='0 0 100 100'
    >
